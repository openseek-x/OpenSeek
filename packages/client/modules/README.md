...
The Node half scans enabled Loader entries for web `dsh.client` packages, resolves each `exports["./client"]`, and hashes the built bundle into the boot graph. With `ctx.webServer` present it serves bundles under `/plugins` and injects the graph into every index response. Without a Web server, a closed application can read `graph()`, resolve an exact bundle with `clientPath(id)`, and call `injectBootManifest()` while serving both through its own origin; the Electron application uses that path. Source launch maps host imports to TypeScript source but still consumes the built client export; missing files share one build instruction followed by a package/path list, while unrelated filesystem errors remain separate failures.
