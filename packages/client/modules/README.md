...
The Node half scans enabled Loader entries for web `dsh.client` packages, resolves each `exports["./client"]`, hashes the built bundle into the boot graph, carries package-specific `dsh.client.external` requests, and orders dynamic providers before consumers. With `ctx.webServer` present it serves bundles with their source maps under `/plugins` and injects the graph into every index response. Without a Web server, a closed application can read `graph()`, resolve an exact bundle with `clientPath(id)`, and call `injectBootManifest()` while serving both through its own origin; the Electron application uses that path. Source launch maps host imports to TypeScript source but still consumes this built client export; missing files share one build instruction followed by a package/path list, while unrelated filesystem errors remain separate failures.
...
- **A non-Web shell owns delivery policy** — `clientPath()` returns the registry-owned built path, but the caller must confine its application URL space, assign response content types, and apply its own content security policy.
