...
Node 侧会扫描已启用的 Loader 配置项以发现 web `dsh.client` 包，解析每个 `exports["./client"]`，把构建后的 bundle 哈希和包专属 `dsh.client.external` 请求写入启动图，并把动态提供方排在消费者之前。存在 `ctx.webServer` 时，它通过 `/plugins` 提供 bundle 及其 sourcemap，并把启动图注入每个 index 响应。没有 Web 服务器时，封闭应用可以读取 `graph()`、通过 `clientPath(id)` 解析确切 bundle，并在自己的 origin 下提供两者时调用 `injectBootManifest()`；Electron 应用采用这条路径。源码启动会把宿主侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出；缺失文件共享一条构建说明，随后以包／路径列表列出各项，而无关的文件系统错误仍是独立故障。
...
- **非 Web 外壳自行负责交付策略**：`clientPath()` 返回注册表拥有的构建路径，但调用方必须限制其应用 URL 空间、设置响应 content type，并应用自己的内容安全策略。
