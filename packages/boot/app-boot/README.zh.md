...
- **以宿主为锚点的裸包需要完整的已安装 peer 闭包**：内部 loader 与 `createRequire` 两条路径都从 `bareModuleBaseUrl` 解析；可移植嵌入方必须把每个运行时包和必需 peer 放在该锚点能够解析的位置。
