...
OpenSeek `0.1.9` Release 会设置 `OPENSEEK_DESKTOP_RELEASE=0.1.9`。该精确值允许生成 ad-hoc 签名的 macOS 应用和未签名的 Windows 安装程序，把已验证的产物发布到 OpenSeek GitHub Release，并配置对应的 GitHub 更新通道。该保护会拒绝其他值，或拒绝与 Desktop manifest 版本不一致的值。
