...
Official macOS and Windows releases use `electron-updater` with build-pinned GitHub Releases metadata. The updater rejects prereleases and downgrades, never requires a GitHub token at runtime, and does not let the renderer choose a feed or release URL. The certificate-free `0.1.1` Mac application carries a stable ad-hoc designated requirement, `identifier "ai.deepseek.harness"`, but no Developer ID identity or notarization; the Windows application and NSIS installer carry no Authenticode signature, and `app-update.yml` omits `publisherName`. Gatekeeper or SmartScreen can therefore warn recipients, and Windows performs no publisher-identity check for this release. Explicit `signed` mode instead pins the certificate's full Subject for Windows and requires the Apple and Windows credentials before packaging. macOS updates run only after the application is installed under `/Applications` or the user's `Applications` directory. Linux archives expose the Releases page but do not support in-place updates.
...
- **The `0.1.1` release is certificate-free** — macOS uses a stable ad-hoc designated requirement without identity authentication or notarization, while Windows omits Authenticode and `publisherName`; operating-system trust warnings remain possible. The source-controlled mode and reviewed-tag setting select this release explicitly, and signed mode still fails when any required credential is absent.
