...
- **The `0.1.1` seed, `0.1.2`, `0.1.3`, `0.1.4`, `0.1.5`, `0.1.6`, `0.1.7`, and `0.1.8` releases are certificate-free** — macOS uses a stable ad-hoc designated requirement without identity authentication or notarization, while Windows omits Authenticode and `publisherName`; operating-system trust warnings remain possible. The source-controlled mode and reviewed-tag setting select `0.1.8` explicitly, and signed mode still fails when any required credential is absent.
