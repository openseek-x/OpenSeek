 ...
 */
export async function signMacOSRuntime(root: string, appId: string, expected: MacOSSigningEnvironment | 'adhoc'): Promise<number> {
  ...
  const workers = Array.from({ length: Math.min(4, files.length) }, async () => {
    for (;;) {
      ...
      if (expected === 'adhoc') {
        execFileSync('/usr/bin/codesign', ['--force', '--sign', '-', '--identifier', identifier, join(root, path)])
        execFileSync('/usr/bin/codesign', ['--verify', '--strict', join(root, path)])
      } else {
