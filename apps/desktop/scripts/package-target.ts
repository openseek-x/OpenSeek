...
import { isOpenSeekRelease } from './desktop-release-environment.mjs'
...
async function main(): Promise<void> {
  const forkRelease = isOpenSeekRelease(process.env)
  ...
  if (target.platform === 'darwin' && !invocation.directory && !forkRelease) {
    await runPnpm([
  ...
  if (!invocation.directory && !invocation.unsigned && !forkRelease) writeReleaseRecord(target, electronBuilderEnv, buildPaths.artifacts)
}
