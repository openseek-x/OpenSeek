...
import {
  isOpenSeekRelease, resolveDesktopAppId,
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (process.platform === 'darwin') {
      await signMacOSRuntime(DSH_OUTPUT_ROOT, resolveDesktopAppId(process.env), isOpenSeekRelease(process.env) ? 'adhoc' : resolveMacOSSigningEnvironment(process.env))
    }
