...
describe('desktop macOS release signature', () => {
  ...
  it('uses the reviewed OpenSeek release version for unsigned GitHub artifacts', async () => {
    const { createElectronBuilderConfig } = await import('../electron-builder.config.mjs')
    const environment = {
      DSH_DESKTOP_APP_ID: RELEASE_ENVIRONMENT.DSH_DESKTOP_APP_ID,
      DSH_DESKTOP_TARGET_PLATFORM: 'darwin',
      DSH_DESKTOP_TARGET_ARCH: 'arm64',
      OPENSEEK_DESKTOP_RELEASE: '0.1.11',
    }
    ...
    expect(config).toMatchObject({
      ...
      dmg: { sign: false },
      publish: [{
        provider: 'github', owner: 'openseek-x', repo: 'OpenSeek', channel: 'latest-arm64',
      }],
    })
  })
