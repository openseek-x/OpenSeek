...
> 分工线：本篇 = 分层模型 + 通道无关的 RPC 协议；协议的 Web 实现由 HTTP 上行加 [WebSocket 下行载体](2026-08-04-websocket-downlink-carrier.zh.md)组成，Electron 实现在[桌面应用笔记](2026-08-14-electron-desktop-application.zh.md)中，浏览器对象层见 [Web 客户端架构笔记](2026-07-19-gui-web-client-architecture.zh.md)。
...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - `apps/desktop`（`@deepseek-ai/dsh-desktop`）通过 structured-clone IPC 复用同一套 Web client 包，并从 `dsh://app/` 提供其构建产物，不需要 Web server。
...
| `DesktopApiClient` | dsh-client-connection，由 apps/desktop 的 preload bridge 选择 | structured-clone unary 往返 + 传输既有 SSE bytes 的流式 IPC | Electron 客户端；发送方／源校验和原生生命周期见[桌面应用](2026-08-14-electron-desktop-application.zh.md) |
