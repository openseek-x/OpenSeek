...
> 分工线：本篇 = 分层模型 + 通道无关的 RPC 协议；协议的 Web 实现由 HTTP 上行加 [WebSocket 下行载体](2026-08-04-websocket-downlink-carrier.md)组成，Electron 实现见[桌面应用笔记](2026-08-14-electron-desktop-application.md)，浏览器对象层见 [Web 客户端架构笔记](2026-07-19-gui-web-client-architecture.md)。
...
- `apps/` 作为对外导出的应用入口，可以由 Client / Host 混合组装。
    ...
    - `apps/desktop`（`@deepseek-ai/dsh-desktop`）经 structured-clone IPC 复用同一套 Web client 包，并从 `dsh://app/` 提供其构建工件，不挂载 Web 服务器。
...
| 应用 | `@deepseek-ai/dsh`（apps/cli）+ `dsh-web-frontend`（apps/web）+ `@deepseek-ai/dsh-desktop`（apps/desktop） | CLI 分发、薄 Vite 入口，以及位于共享 Web profile 之上的 Electron main／preload 组装 | 应用持有平台生命周期与工件位置；桌面应用替换 HTTP 载体而不改变客户端组合 |
...
1. **选择 Fetch 实现**：浏览器同源 HTTP／WebSocket、进程内 Host 分发器，或随附 Electron IPC 载体这类传输子类（见下文「子类表」）。
2. **在 `apps/` 下写拼装模块**：`startHost()` + 客户端子类 + 该应用私有的信号/打印/退出语义；混合体不建包，拼装写在 app 里。
3. **需要 HTTP 承载才 import `dsh-host-webserver`**，否则零端口。

随附应用保持这一区分：Web 应用挂载 Host、HTTP／WebSocket 载体与浏览器组合；桌面应用挂载相同 Host／client 组合，使用 IPC 且不开放端口；`dsh --profile headless` 挂载直接使用核心服务的 runner，不包含 Host、GUI、HTTP 或端口。ACP 类协议桥不遵循 client 载体清单：它把 core 暴露给外部生态，直接通过 `ctx.plugin(入口插件)` 挂载，不使用 Fetch。
...
| `DesktopApiClient` | dsh-client-connection，由 apps/desktop preload 桥选择 | structured-clone unary 往返 + 承载现有 SSE 字节的流式 IPC | Electron 客户端；sender／origin 校验和原生生命周期见[桌面应用](2026-08-14-electron-desktop-application.md) |
