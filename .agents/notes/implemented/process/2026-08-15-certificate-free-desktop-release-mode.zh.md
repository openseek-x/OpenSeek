...
公开桌面更新路径已有 `0.1.1` seed 与 `0.1.2` 后继版本，而 Apple Developer ID、notarization 与 Windows Authenticode 凭据在 `0.1.3` 发布时仍不可用。如果把缺少 secret 当作生成无证书产物的许可，后续 Release 的凭据配置一旦损坏，就会静默削弱其安全性。若无证书打包只允许手动运行，普通 `dsh-v0.1.3` 标签也无法执行四平台校验与原子的 GitHub Release 发布流程。
...
Mac 包会获得 deep ad-hoc 签名，外层 designated requirement 固定为 `identifier "ai.deepseek.harness"`。工作流会按照同一 requirement 校验应用及解压后的更新 ZIP。这样可以维持更新器跨 `0.1.1`、`0.1.2` 与 `0.1.3` 所需的 requirement 匹配，但不会认证开发者身份，也不会执行 Apple notarization。Gatekeeper 可能警告或阻止接收方，直到用户显式允许该应用。
...
**让以后所有标签都保持无证书。** 这可以省去版本专用编辑，却会让后续 Release 未经审查就继承较弱的身份策略。配置的无证书标签会具名 `dsh-v0.1.3`，Release 检查还会独立要求该标签等于桌面 manifest 版本。

## Consequences

`dsh-v0.1.3` 可以在没有平台证书的情况下执行普通标签触发构建，并完成从 `0.1.2` 的更新路径。所有架构仍会在一份完整的公开 Release 出现前成功，signed 实现也会保留，供未来显式切换。
