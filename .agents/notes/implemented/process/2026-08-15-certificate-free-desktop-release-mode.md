...
**Allow every future tag to remain certificate-free.** This would avoid a version-specific edit, but it would let a later release inherit the weaker identity policy without review. The configured certificate-free tag names `dsh-v0.1.1`, and the release check independently requires that tag to equal the desktop manifest version.

## Consequences

`dsh-v0.1.1` can exercise the ordinary tag-triggered build and complete the seed-to-next-version update test without platform certificates. All architectures still succeed before one complete public Release appears, and the signed implementation remains present for an explicit future switch.
