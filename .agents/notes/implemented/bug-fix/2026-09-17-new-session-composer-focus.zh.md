...
一个 Workspace 会有意保留一个尚未提交消息的 blank Session。当这个 blank Session 已被选中时，全局「新建会话」操作会再次打开同一个身份。Conversation 树不会重新挂载，浏览器焦点仍停留在被点击的按钮上，页面也没有其他变化。因此，尽管 blank Session 已可输入，操作看起来仍像没有响应。没有 Workspace 的新会话页面也缺少相同的焦点反馈。
...
新会话对 blank Session 数据仍保持幂等，而每次成功激活都会把键盘焦点移到可操作表面。复用 blank 时不会改变草稿或 Session 身份。焦点交付只有一个所有者，会保留 Lexical 选区，并处理导航先于 React 挂载新 composer 完成的情况。

Conversation 服务测试覆盖已挂载与延迟绑定，InputBar 测试覆盖 editor 聚焦和 disposer 清理，Workspace 测试覆盖复用 blank 与无 Workspace target。打包 Desktop 流程验证：在已选中的 blank 上点击「新建会话」后，焦点会从按钮移到 composer。
