...
[`dsh-base`](../packages/bundle/base/README.zh.md) 是每个 profile 的第一层：模型适配器、工具、持久化、沙箱与审批策略、设置、凭据、遥测。[`dsh-web-app`](../packages/bundle/web-app/README.zh.md) 增加浏览器和 [Electron 桌面](../apps/desktop/README.zh.md)应用使用的图形客户端组合；桌面 overlay 会替换它的 HTTP/浏览器专用条目。[`dsh-headless`](../packages/bundle/headless/README.zh.md) 增加一次性运行器，且完全不带服务器。
