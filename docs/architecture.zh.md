...
[`dsh-base`](../packages/bundle/base/README.md) 是每个 profile 的第一层：模型适配器、工具、持久化、沙箱与审批策略、设置、凭据、遥测。[`dsh-web-app`](../packages/bundle/web-app/README.md) 增加由浏览器与 [Electron 桌面](../apps/desktop/README.md)应用共用的图形客户端组合；桌面 overlay 会替换其中的 HTTP／浏览器专用条目。[`dsh-headless`](../packages/bundle/headless/README.md) 增加一次性运行器，且完全不带服务器。
