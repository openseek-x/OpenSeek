...
[`dsh-base`](../packages/bundle/base/README.md) is the first layer of every profile: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`dsh-web-app`](../packages/bundle/web-app/README.md) adds the graphical client composition used by both the browser and [Electron desktop](../apps/desktop/README.md) applications; the desktop overlay replaces its HTTP/browser-specific rows. [`dsh-headless`](../packages/bundle/headless/README.md) adds a one-shot runner with no server at all.
