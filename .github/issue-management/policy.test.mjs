...
test('keeps trusted preflight before credential-gated upstream project policy', () => {
  ...
  assert.ok(steps[2].includes('id: credentials'))
  assert.ok(steps[2].includes('APP_PRIVATE_KEY: ${{ secrets.DSH_ISSUE_APP_PRIVATE_KEY }}'))
  assert.ok(steps[3].includes("if: ${{ steps.preflight.outputs.needs-project == 'true' && steps.credentials.outputs.configured == 'true' }}"))
  ...
  assert.ok(steps[4].includes("if: ${{ steps.preflight.outputs.legacy-automated == 'true' || steps.credentials.outputs.configured == 'true' }}"))
})
...
test('runs trusted rollout selection with absent and present capability markers', { skip: process.platform === 'win32' ? 'The policy workflow executes under hosted Ubuntu bash' : false }, (t) => {
  ...
  const source = readFileSync(new URL('../workflows/issue-policy.yml', import.meta.url), 'utf8')
  const script = source.split('        run: |\n')[1].split('      - name: Detect project credentials')[0]
